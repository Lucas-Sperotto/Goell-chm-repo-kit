...
run_table1() {
  ensure_build
  "$PYTHON_BIN" src/reproduce_table1.py \
...
run_figure() {
  ...
  FIG_A_OVER_B="${FIG_A_OVER_B:-${defaults[0]}}"
  FIG_NR="${FIG_NR:-${defaults[1]}}"
  FIG_TITLE="${FIG_TITLE:-${defaults[2]}}"
  FIG_BMIN="${FIG_BMIN:-0}"
  FIG_BMAX="${FIG_BMAX:-4}"
  ...
  "$PYTHON_BIN" src/validate_goell.py \
...
run_principal_sweep() {
  ...
  "$PYTHON_BIN" src/sweep_principal_modes.py \
    --tag "$tag" \
    --vary "$vary" \
    --values "$values" \
...
run_validate() {
  ...
  "$PYTHON_BIN" src/analyze_table1_variation.py
